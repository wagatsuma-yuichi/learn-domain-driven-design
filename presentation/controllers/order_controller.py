...
from application.interfaces.order_use_case import OrderCreationInputBoundary, OrderQueryInputBoundary, OrderManagementInputBoundary
from presentation.presenters.order_presenter import OrderCreationPresenter, OrderQueryPresenter, OrderManagementPresenter, OrderErrorOutputBoundary
from fastapi import APIRouter, Depends, HTTPException, status
from pydantic import BaseModel, Field
from application.usecases.dependancies import order_add_usecase
...
def create_order(request_data: OrderRequest, order_use_case: Annotated[OrderCreationInputBoundary, Depends(order_add_usecase)], presenter: OrderCreationPresenter = Depends()) -> Dict[str, Any]:
    """注文を作成する"""
    try:
        # リクエストデータからDTOを作成
...
def get_order(order_id: str, order_use_case: OrderQueryInputBoundary = Depends(), presenter: OrderQueryPresenter = Depends()) -> Dict[str, Any]:
    """注文を取得する"""
    try:
        # 注文IDをUUIDに変換
...
def get_customer_orders(customer_id: str, order_use_case: OrderQueryInputBoundary = Depends(), presenter: OrderQueryPresenter = Depends()) -> Dict[str, Any]:
    """顧客の注文を取得する"""
    try:
        # 顧客IDをUUIDに変換
...
def update_order_status(order_id: str, status_update: OrderStatusUpdate, order_use_case: OrderManagementInputBoundary = Depends(), presenter: OrderManagementPresenter = Depends()) -> Dict[str, Any]:
    """注文ステータスを更新する"""
    try:
        # 注文IDをUUIDに変換
        order_uuid = UUID(order_id)
...
def cancel_order(order_id: str, order_use_case: OrderManagementInputBoundary = Depends(), presenter: OrderManagementPresenter = Depends()) -> Dict[str, Any]:
    """注文をキャンセルする"""
    try:
        # 注文IDをUUIDに変換
        order_uuid = UUID(order_id)
